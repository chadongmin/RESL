...
#define SHORT_ADDR 0x3441 /* "A1" (31 = 1, 32 = 2, 33 = 3, 41 = A) 앵커의 주소. x86 CPU는 리틀 엔디언이므로 순서가 바뀜*/
#define SRC_ADDR   0x4556//0x4556 /* "VE" (56 = V, 45 = E) Source Addr(상대방의 Addr)*/
...
#define RX_BUF_LEN 40 // Must be less than FRAME_LEN_MAX_EX
...
int ss_twr_responder(void)
{
    ...
    /* 자동 ACK 설정. (첫 번째 매개변수는 ACK 딜레이 시간. 0이므로 a.s.a.p) */
    ...
    while (1)
    {
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            dwt_readrxdata(udp_msg, 26, 12);
            ///test_run_info(udp_msg);
            //test_run_info2(udp_msg, 9);
//            test_run_info2(&udp_msg[9], 9);
//            test_run_info2(&udp_msg[18], 9);


    		ethernetif_input(&gnetif);
    		sys_check_timeouts();
    		udp_echoclient_send(udp_msg);
    		//udp_echoclient_send(&udp_msg[9]);
    		//udp_echoclient_send(&udp_msg[18]);
        }
        //test_run_info(udp_msg);
        //test_run_info(udp_msg[9], 23);
